    ...
    def connect_neighborhoods(self,neighborhood):
        #To do, find something better
         for i in range(len(self.populations)):
             for j in range(i):
                mean_number_of_contacts = min(self.populations[j].sum(),self.populations[i].sum())/100
...
if __name__ == "__main__":
    ...
    agedist = np.loadtxt("age_distributions/United_States_subnational_Massachusetts_age_distribution_85.csv", delimiter =',')
